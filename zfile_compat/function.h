...
size_t _zfile_get_file_size(uint64_t fd) {
#ifndef __KERNEL__
  struct stat stat;
  fstat(fd, &stat);
  return stat.st_size;
...
int _zfile_fstat(uint64_t fd, void *stat) {
#ifndef __KERNEL__
...
ssize_t _zfile_pread(uint64_t fd, void *buf, size_t n, off_t offset) {
#ifndef __KERNEL__
...
void *_zfile_malloc(size_t size) {
#ifndef __KERNEL__
  return malloc(size);
#else
...
void *_zfile_realloc(void *ptr, size_t size) {
#ifndef __KERNEL__
